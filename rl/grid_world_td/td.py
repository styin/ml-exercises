...
def epsilon_greedy_policy(nS, nA, Q_function, eps=0.5):
    ...
    for s in range(nS):
        # the probability of taking a non-best action is distributed uniformly to (nA - 1) actions
        policy[s] = eps / (nA - 1)          #! potential issue with implementation is division by zero when nA = 1
        # update the probability of taking the best action to be 1 - eps
        policy[s][Q_function[s].argmax()] = 1 - eps
    ############################
...
def sample_action(policy, state):
    ...
    ############################
    # YOUR IMPLEMENTATION HERE 
    # sample from indices of the policy with the given probabilities with np.arange(len(policy[state]))
...
def Q_learning_step(Q_function, state, action, reward, next_state, next_action, terminal, alpha, gamma):
    ...
    ############################
    # YOUR IMPLEMENTATION HERE #
    sample_target = reward
    if not terminal:
        sample_target += gamma * Q_function[next_state].max()
    next_Q_function[state][action] = (1 - alpha) * Q_function[state][action] + alpha * sample_target
    ############################
...
def Sarsa_step(Q_function, state, action, reward, next_state, next_action, terminal, alpha, gamma):
    ...
    sample_target = reward
    if not terminal:
        sample_target += gamma * Q_function[next_state][next_action]
    next_Q_function[state][action] = (1 - alpha) * Q_function[state][action] + alpha * sample_target
    ############################
...
if __name__ == "__main__":
    ...
    # for alpha in alphas:
    #     for gamma in gammas:
    #         print("\nalpha = {}, gamma = {}".format(alpha, gamma))
    #         print("-" * 25 + "\nBeginning Q Learning\n" + "-" * 25)
    #         Q_function, p_q_learning = learn(Q_learning_step, alpha=alpha, gamma=gamma)
    #         render_single(env, p_q_learning)
    #         print(p_q_learning)
    #         print("\n" + "-" * 25 + "\nBeginning Sarsa\n" + "-" * 25)
    #         Q_function, p_sarsa = learn(Sarsa_step, alpha=alpha, gamma=gamma)
    #         render_single(env, p_sarsa)
    #         print(p_sarsa)
