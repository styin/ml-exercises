# # only run one iteration of policy evaluation
    # # for the given policy and return the value function
    # # value_function = np.zeros(nS)
    # for state in range(nS):
    #   for action in range(nA):
    #     for prob, next_state, reward, terminal in P[state][action]:
    #       # terminate policy evaluation when max |value_function(s) - prev_value_function(s)| < eps
    #       if 
    #       value_function[state] += policy[state] * prob * (reward + gamma * value_function[next_state])
...
def policy_evaluation(P, nS, nA, policy, gamma=0.9, eps=1e-3):
    ...
    sweep = 0
    while True:
        # print("Policy Evaluation Sweep: ", sweep)
        sweep += 1
        delta = 0.0
        for state in range(nS):
            previous_value = value_function[state]
            # get the action for the state from the policy to be evaluated
            action = policy[state]
            new_value = 0
            # for each possible transition from the state with the action
            for p_trans, next_state, reward, _ in P[state][action]:
                # Bellman equation: V(s) = Σ T * [r + γV(s')]
                new_value += p_trans * (reward + gamma * value_function[next_state]) 
            value_function[state] = new_value
            # terminate policy evaluation when max |value_function(s) - prev_value_function(s)| < eps
            # intuitively: if the value function is not changing much, then we have converged
            delta = max(delta, abs(previous_value - value_function[state]))
        if delta < eps:
            break 
    ############################
...
def policy_improvement(P, nS, nA, value_from_policy, policy, gamma=0.9):
    ...
    for state in range(nS):
        ...
        # we calculate the Q values for each action in the state
        for action in range(nA):
            # for each possible transition from the state with the action
            for prob, next_state, reward, _ in P[state][action]:
                # Bellman equation: Q(s, a) = Σ p(s', r|s, a) * [r + γV(s')]
                q_values[action] += prob * (reward + gamma * value_from_policy[next_state])
...
def policy_iteration(P, nS, nA, gamma=0.9, eps=10e-3):
    ...
    while True:
        ...
        # print("Policy Improvement Sweep: ", sweep)
        new_policy = policy_improvement(P, nS, nA, value_function, policy, gamma)
        # if the policy has not changed, we have converged
...
def value_iteration(P, nS, nA, gamma=0.9, eps=1e-3):
    ...
    while True:
        sweep += 1
        # print("Value Iteration Sweep: ", sweep)
        delta = 0.0
        # for each state
        for state in range(nS):
            q_values = np.zeros(nA)
            ...
            for action in range(nA):
                # for each possible transition from the state with the action
                for prob, next_state, reward, _ in P[state][action]:
                    # V(k+1)(s) = max Σ T * [r + γV(k)(s')] = max(Q(s, a))
                    q_values[action] += prob * (reward + gamma * value_function[next_state])
            # we update the value function with the maximum Q value
            value_function[state] = np.max(q_values)
            delta += max(delta, abs(previous_value - value_function[state]))
        # terminate value iteration when max |value_function(s) - prev_value_function(s)| < eps
        if delta < eps:
            break
    # we extract the final converged policy from the value function
